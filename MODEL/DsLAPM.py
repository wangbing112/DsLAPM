...
class DsLAPMConfig(BaseSettings):
    name: Literal["dslapm"]
    conv_layers: int = 3
...
class BaseDsLAPMConv(MessagePassing):

    def __init__(self, fc_features):
        super(BaseDsLAPMConv, self).__init__(node_dim=0)
        self.bn = nn.BatchNorm1d(fc_features)  # 批归一化层的输入大小为fc_features
    ...
    def forward(self, x, edge_index, edge_attr):
        s = x.size(0)
        out = self.propagate(
            edge_index, x=x, edge_attr=edge_attr, size=(s, s)
        )
        b = self.bn(out)
        f = x + b
        return F.relu(f)
    ...
    def message(self, x_i, x_j, edge_attr, index):
        concatenated_features = torch.cat((x_i, x_j, edge_attr), dim=1)
        score = torch.sigmoid(self.bn_interaction(self.nonlinear_full(concatenated_features)))
        return score * self.nonlinear(torch.cat((x_i, x_j, edge_attr), dim=1))

class DsLAPMConv(BaseDsLAPMConv):
    def __init__(self, fc_features):
        super(DsLAPMConv, self).__init__(fc_features)

class angle_DsLAPMConv(BaseDsLAPMConv):
    def __init__(self, fc_features):
        super(angle_DsLAPMConv, self).__init__(fc_features)

class FourierFeatureMapping(nn.Module):
    def __init__(self, num_frequencies):
        super(FourierFeatureMapping, self).__init__()
        self.num_frequencies = num_frequencies
        self.freqs = 2 * math.pi * torch.arange(1, num_frequencies + 1)

    def forward(self, angles):
        angles = angles.unsqueeze(-1)
        freqs = self.freqs.to(angles.device)
        features = torch.cat([torch.sin(freqs * angles), torch.cos(freqs * angles)], dim=-1)
        return features

class DsLAPM(nn.Module):

    def __init__(self, config: DsLAPMConfig = DsLAPMConfig(name="dslapm")):
        super().__init__()
        self.config = config
        ...
        self.angle_conv_layers = angle_DsLAPMConv(config.fc_features)

        self.angle_embedding = nn.Sequential(
            FourierFeatureMapping(num_frequencies=5),
            nn.Linear(2 * 5, config.fc_features),
            nn.SiLU(),
        )
    ...
    def forward(self, data, print_data=False):
        ...
        if self.config.euclidean:
            edge_features_a = self.edge_embedding(data.edge_attr_a)
        else:
            edge_features_a = self.edge_embedding(-0.75 / data.edge_attr_a)

        angle_index = data.angle_index
        angle_attr = data.angle_attr
        angle_attr = angle_attr.float()
        angle_attr = self.angle_embedding(angle_attr)

        edge_features_a = self.angle_conv_layers(edge_features_a, angle_index, angle_attr)
        ...
        if not self.config.euclidean and not self.config.transformer:
            ...
            edge_index = torch.cat([edge_index, edge_index_a], 1)
            edge_features = torch.cat([edge_features, edge_features_a], 0)
        ...
        return torch.squeeze(self.fc_out(features)), features
