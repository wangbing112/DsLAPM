...
from models.DsLAPM import DsLAPM
...
def train_pyg(
    ...
    def log_results(engine):
        ...
        if config.progress:
            ...
            pbar.log_message(f"Epoch: {current_epoch}")
    ...
    with torch.no_grad():
        ...
        for dat, id in tqdm(zip(test_loader, ids)):
            ...
            out_data, output_fea = net(data.to(device))
            # print("out_data = net(data.to(device)): ", out_data)

            out_data = out_data.cpu().numpy().tolist()
            output_fea = output_fea.squeeze().cpu().numpy()
            out_fea_list.append(output_fea)
            # print("out_data = out_data.cpu().numpy().tolist()", out_data)
            target = target.cpu().numpy().flatten().tolist()
            # print("target = target.cpu().numpy().flatten().tolist()", target)
    ...
    z = manifold.TSNE(n_components=2, init='pca').fit_transform(out_fea_list) # or: random
    fig, ax = plt.subplots(figsize=(10, 8))
    scatter = ax.scatter(z[:, 0], z[:, 1], s=15, c=targets, alpha=0.8, cmap=cmap)
    ...
    plt.savefig("dft_ehull.png", format='png')
    plt.show()

    # 默认write_predictions为True
    if config.write_predictions:
        f.close()
    t2 = time.time()
...
def train_prop_model(config: Dict, data_root: str = None, checkpoint: str = None, testing: bool = False, file_format: str = 'poscar'):
    if config["dataset"] == "megnet":
        ...
        if config["target"] == "e_form" or config["target"] == "gap pbe":
            config["n_train"] = 60000   # 默认none
            config["n_val"] = 5000      # none
            config["n_test"] = 4239     # none
